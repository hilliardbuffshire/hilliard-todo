...
> 마지막 업데이트: 2026-05-24 | Commit: `bd3fd64c479d` | (responsive-fix + dead-code-cleanup)
...
| 2026-05-24 | `bd3fd64c479d` | **responsive-fix + dead-code-cleanup**: ①`#mobileCalHead` CSS 이중 `display:flex` 버그 수정 → PC에서 모바일 헤더 중복 노출 제거. ②`@media(max-width:768px)`에 `.calHead{display:none!important}` 추가 → 모바일에서 PC 헤더 숨김. ③`body.desk-mode .calHead{display:flex!important}` 추가 → PC보기 버튼 완전 작동. ④`renderAll()`의 `renderAgenda()` 조건 개선 (`_agEl.offsetParent||window.innerWidth<=768`). ⑤데드코드 삭제 -11KB: 7개 숨겨진 뷰 HTML(vToday/vAll/vDone/vRecurring/vGoals/vIdeas/weeklyTmplModal) + CSS 고아 규칙(#mobileBottomNav/.psStrip/.psBtn/.psSel/.psApplyBtn/.navGroupHead/.navSect/.mbNavBtn 등 29개 규칙) 완전 제거. 브라우저 검증: PC(1345px)·모바일(375px) ALL PASS. |
| 2026-05-24 | `ea07542b6bb5` | **fix: meals/workouts onSnapshot → scheduleRender**: 식단·운동 데이터 로드 시 캘린더 즉시 리렌더(기존 curView 조건부로 renderMeal()/renderWorkout() 호출 → scheduleRender()로 교체). 기존 사용자 데이터 캘린더 복구. |
